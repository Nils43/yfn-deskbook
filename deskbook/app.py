...
def current_user():
    """Liefert den aktuell eingeloggten User oder None."""
...
def login_required(f):
    """Decorator: Route wird nur ausgefuehrt wenn User eingeloggt ist.
    Sonst: JSON-API kriegt 401, normale Page kriegt Redirect zu /login."""
    @wraps(f)
    def decorated(*args, **kwargs):
        if current_user() is None:
            # Wenn der Request JSON erwartet (API-Call), JSON zurueckgeben
            if request.path.startswith("/api/"):
                return jsonify({"error": "Authentication required"}), 401
            # Sonst: Redirect zur Login-Seite
...
def create_reservation():
    ...
    reservation = Reservation(
        ...
        user_id=user.id,  # ownership: an aktuellen User binden
        date=data.get("date"),
...
def get_reservations():
    """Liefert nur die Reservierungen des aktuellen Users."""
    user = current_user()
...
def delete_reservation(reservation_id):
    ...
    # Ownership-Check: User darf nur eigene Reservierungen loeschen
